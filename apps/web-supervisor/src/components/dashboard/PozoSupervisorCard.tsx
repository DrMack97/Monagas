  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 text-xs text-slate-500 mt-3 pt-3 border-t border-slate-800">
        <span>🛢️ {pozo.tanques?.length ?? 0} tanque{pozo.tanques?.length === 1 ? '' : 's'}</span>
        <span>👷 {operadoresAsignados} operador{operadoresAsignados === 1 ? '' : 'es'}</span>
        <span>⚠️ {pozo.limResorte} psi / {pozo.limGamma} inH₂O</span>
      </div>
    </button>
  )
}
